...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const file = await tenantStorageService.getProtectedFileStream(apiSession.session, id, "thumb", Number(pageNumber));
  if (!file) {
    return jsonError("Thumbnail wurde nicht gefunden.", 404, "INVOICE_FILE_NOT_FOUND");
  }
